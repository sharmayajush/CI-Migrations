...
func upDataMigration(tx *sql.Tx) error {
	...
	rows, err := tx.Query("SELECT * FROM cwpp.old_channel_slack_settings")
	if err != nil {
		fmt.Println(err)
		return err
	}
	...
	for _, settings := range cache {
		...
		err = tx.QueryRow("SELECT * FROM cwpp.old_channels WHERE channel_id=$1 AND tenant_id=$2", settings.ChannelID, settings.TenantID).Scan(&channel.ChannelID, &channel.ChannelName, &channel.Status, &channel.TenantID, &channel.CreatedBy)
		if err != nil {
			fmt.Println("4----")
			return err
		}
	...
	rows, err = tx.Query("SELECT * FROM cwpp.old_channel_splunk_settings")
	if err != nil {
		fmt.Println(err)
		return err
	}
	...
	rows, err = tx.Query("SELECT * FROM cwpp.old_channel_cloudwatch_settings")
	if err != nil {
		fmt.Println(err)
		return err
	}
	...
	rows, err = tx.Query("SELECT * FROM cwpp.old_channel_jira_settings")
	if err != nil {
		fmt.Println(err)
		return err
	}
	...
	rows, err = tx.Query("SELECT * FROM cwpp.old_channel_rsyslog_settings")
	if err != nil {
		fmt.Println(err)
		return err
	}
